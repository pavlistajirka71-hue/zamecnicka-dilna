...
// Aplikace se přihlašuje jako konkrétní lidský Google účet (OAuth), ne jako servisní
...
export async function ulozitPripojeni(refreshToken, email) {
  ...
  if (error) throw new Error("Uložení přihlášení ke Google Drive se nepovedlo: " + error.message);
}

export async function odpojitDrive() {
  const supabase = getSupabaseAdmin();
  const { error } = await supabase.from("google_drive_auth").upsert({ id: 1, refresh_token: null, connected_email: null, connected_at: null });
  if (error) throw new Error("Odpojení Google Drive se nepovedlo: " + error.message);
}
...
async function getAccessToken() {
  ...
  if (!refreshToken) {
    throw new Error("Google Drive není propojený — v aplikaci klikni na 'Připojit Google Drive' a přihlas se.");
  }
  ...
  if (!res.ok) {
    ...
    // Nejčastější příčina: aplikace (v testovacím režimu Google projektu) má propojení
    // platné jen ~7 dní — po vypršení je potřeba se v aplikaci znovu přihlásit.
    if (text.includes("invalid_grant")) {
      try {
        await odpojitDrive(); // aplikace teď v panelu správně ukáže "nepřipojeno" místo tichého selhávání
      } catch (e) {
        console.error("Označení Drive jako odpojeného se nepovedlo:", e);
      }
      throw new Error("Propojení s Google Drive vypršelo — v aplikaci klikni na 'Připojit Google Drive' a přihlas se znovu.");
    }
...
export async function nahratNaGoogleDrive(bytes, filename, mimeType, slozkaZakazky) {
  ...
  return {
    fileId,
    // Pro vkládání jako <img> (náhledy fotek) — spolehlivější pro přímé zobrazení
    // než starší "uc?export=view", které Google poslední dobou čím dál víc omezuje.
    previewUrl: `https://drive.google.com/thumbnail?id=${fileId}&sz=w1600`,
    // Pro otevření souboru v novém okně (funguje pro jakýkoliv typ souboru, včetně PDF).
