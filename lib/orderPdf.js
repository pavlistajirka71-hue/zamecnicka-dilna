...
// Aplikace si vždycky nahrává vlastní font (přibalený v repozitáři, plná podpora češtiny) a
...
// Storage). Při jakémkoliv selhání vrátí null — aplikace pak obrázek v PDF prostě vynechá,
...
export async function generovatPdfZakazky(order, nastaveni, supabaseAdmin) {
  ...
  if (polozky.length > 0) {
    const celkem = computeKalkulaceCelkem(polozky, nastaveni, order.sazbaDph ?? DPH_SAZBA);
  ...
  if (polozky.length > 0) {
    ...
    doc
      .font("TextBold")
      .fontSize(13)
      .fillColor(nv.zisk >= 0 ? "#3D3D3F" : "#B33A3A")
