...
// ---------- Přehled (peníze/marže, vytíženost) ----------

// Obrat za měsíc — součet cen zakázek VYTVOŘENÝCH ten měsíc. Neúspěšné nabídky
// appka nepočítá, protože z nich žádný obrat nevznikl.
export function obratZaMesic(orders, mesicPrefix) {
  return (orders || [])
    .filter((o) => o.vytvoreno && o.vytvoreno.startsWith(mesicPrefix) && o.stav !== "neuspesnaNabidka")
    .reduce((s, o) => s + (Number(o.cena) || 0), 0);
}

// Hodnota právě rozpracovaných zakázek — kolik peněz appka "má rozdělané", bez
// ohledu na měsíc (appka to bere jako aktuální momentku, ne historii).
export function hodnotaRozpracovanych(orders) {
  return (orders || []).filter((o) => o.stav === "probiha").reduce((s, o) => s + (Number(o.cena) || 0), 0);
}

// Průměrná hodnota zakázky vytvořené ten měsíc (neúspěšné nabídky appka opět
// nepočítá — nezkreslovaly by průměr směrem dolů).
export function prumernaHodnotaZakazky(orders, mesicPrefix) {
  const relevantni = (orders || []).filter((o) => o.vytvoreno && o.vytvoreno.startsWith(mesicPrefix) && o.stav !== "neuspesnaNabidka");
  if (relevantni.length === 0) return 0;
  return relevantni.reduce((s, o) => s + (Number(o.cena) || 0), 0) / relevantni.length;
}

// Porovná PLÁNOVANOU marži (z kalkulace — computeKalkulaceCelkem) se SKUTEČNOU
// (computeNakladyZakazky — podle opravdu odpracovaných hodin a zapsaných nákladů)
// u zakázek vytvořených ten měsíc, které mají kalkulaci. Appka tak jde vidět, jestli
// se odhad v kalkulaci v praxi drží, nebo jestli zakázky "utíkají" do ztráty.
export function planVsSkutecnostMarze(orders, nastaveni, mesicPrefix) {
  return (orders || [])
    .filter((o) => o.vytvoreno && o.vytvoreno.startsWith(mesicPrefix) && normalizovatKalkulaci(o.kalkulace).length > 0)
    .map((o) => {
      const polozky = normalizovatKalkulaci(o.kalkulace);
      const planovana = computeKalkulaceCelkem(polozky, nastaveni, o.sazbaDph ?? DPH_SAZBA);
      const skutecnost = computeNakladyZakazky(o, nastaveni);
      return {
        order: o,
        planovanaMarzeKc: planovana.marzeKc,
        planovanaMarzePct: planovana.marzePct,
        skutecnaMarzeKc: skutecnost.zisk,
        skutecnaMarzePct: skutecnost.marzePct,
      };
    });
}

// Plán hodin (z kalkulace) vs. skutečně odpracované hodiny u HOTOVÝCH/FAKTUROVANÝCH
// zakázek — appka to bere jako "posledních N dokončených", ne podle měsíce: appka
// totiž nikde neeviduje datum REÁLNÉHO dokončení (jen datum vytvoření), takže by
// měsíční pohled klidně ukázal zakázku založenou v jiném měsíci, než byla hotová.
export function planVsSkutecnostHodin(orders, limit = 20) {
  return (orders || [])
    .filter((o) => o.stav === "hotovo" || o.stav === "fakturovano")
    .slice()
    .sort((a, b) => (b.vytvoreno || "").localeCompare(a.vytvoreno || ""))
    .slice(0, limit)
    .map((o) => {
      const plan = planHodin(o);
      const skutecnostDilna = sumHodin(o.prace, "dilna");
      const skutecnostMontaz = sumHodin(o.prace, "montaz");
      const skutecnost = skutecnostDilna + skutecnostMontaz;
      return { order: o, plan, skutecnost, skutecnostDilna, skutecnostMontaz, rozdil: skutecnost - plan };
    });
}

// Odpracované hodiny za měsíc podle pracovníka (dílna/montáž zvlášť) — appka to
// bere napříč VŠEMI zakázkami, ať jde vidět vytíženost lidí, ne jen zakázek.
export function hodinyPodlePracovnika(orders, mesicPrefix) {
  const mapa = new Map();
  (orders || []).forEach((order) => {
    (order.prace || []).forEach((entry) => {
      if (!entry.datum || !entry.datum.startsWith(mesicPrefix)) return;
      const hodiny = Number(entry.hodiny) || 0;
      if (hodiny <= 0) return;
      const jmeno = (entry.pracovnik || "").trim() || "(bez jména)";
      if (!mapa.has(jmeno)) mapa.set(jmeno, { pracovnik: jmeno, dilna: 0, montaz: 0 });
      const typ = entry.typ === "montaz" ? "montaz" : "dilna";
      mapa.get(jmeno)[typ] += hodiny;
    });
  });
  return Array.from(mapa.values())
    .map((p) => ({ ...p, celkem: p.dilna + p.montaz }))
