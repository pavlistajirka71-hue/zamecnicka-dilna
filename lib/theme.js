...
export const STATUSES = [
  ...
  { key: "hotovo", label: "Zaplaceno", color: C.moss },
  { key: "fakturovano", label: "Fakturováno", color: C.brass },
  { key: "neuspesnaNabidka", label: "Neúspěšná nabídka", color: C.neutral },
];
...
export const DEFAULT_NASTAVENI = {
  sazbaDilna: 550,
  sazbaMontaz: 650,
  // Nákladová sazba — aplikace to používá pro SKUTEČNÉ náklady zakázky (Vyhodnocení),
  // ne pro to, co se počítá zákazníkovi. Typicky nižší než sazbaDilna/sazbaMontaz
  // výše (ty jsou zákaznická cena hodiny v kalkulaci/nabídce), protože aplikace tam
  ...
  vybraniUzivatele: [], // id uživatelů aplikace, kteří se mají taky nabízet (prázdné = žádný)
};
...
export function novyProtokol(order, nastaveni) {
  return {
    ...
    opravaZhotoviteleDatum: null,
    opravaUdajuDatum: null,
    podpisPath: null,
    podpisDatum: null,
    stav: "rozpracovany",
  };
...
export function computeNakladyZakazky(order, nastaveni) {
  ...
  // Když aplikace nemá vytvořenou kalkulaci, cenaBezDph z ní vyjde 0 — aplikace by pak
  // "skutečný výsledek zakázky" počítala vůči nulovému příjmu, i když má zakázka
  // reálnou cenu vyplněnou přímo (bez kalkulace). Aplikace proto v tom případě
  // spadne na skutečnou cenu zakázky, stejně jako aplikace dělá i jinde (export do
...
// Aplikace hledá číslo zakázky (Z-2026-0008 nebo i podzakázku Z-2026-0008-1) KDEKOLIV
// v textu — nezáleží, jestli ho aplikace najde ve variabilním symbolu, poznámce nebo
// popisu faktury. Aplikace se totiž nespoléhá na to, do jakého konkrétního pole ve
...
// zakázky v JAKÉMKOLIV sloupci — aplikace tak nemusí vědět předem, do kterého pole si
// ho ve Flexi zapisuješ.
export function najitCisloZakazkyVRadku(radek) {
  // Přednostně aplikace hledá přímo sloupec "číslo zakázky" / "zakázka" — to je
  ...
  // Záložně (kdyby aplikace název sloupce nerozpoznala) prohledá celý řádek.
...
// Spáruje naimportované řádky faktur se zakázkami aplikace podle nalezeného čísla.
// Vrací zvlášť spárované (aplikace ví, kam náklad patří) a nespárované (aplikace nenašla
// číslo zakázky, nebo takové číslo aplikace nezná) — ty musí uživatel doplnit ručně.
...
// Aplikace nezná předem přesný tvar CSV exportu z Flexi (liší se podle nastavení
...
// Celý řádek se ale aplikaci zachová beze změny, ať v něm najitCisloZakazkyVRadku
// může hledat číslo zakázky i ve sloupci, který aplikace neuhodla.
...
// Z libovolného tvaru řádku (různé aplikace/exporty nazývají sloupce jinak) aplikace
...
// Rozpozná "1 234,50 Kč" / "1234.5" / "2 000" a vrátí číslo — aplikace nezná přesně,
// jak Flexi částky formátuje, takže je radši ke všem obvyklým tvarům shovívavá.
export function parseCastku(text) {
  if (!text && text !== 0) return 0;
  let cislo = String(text).replace(/[^\d,.-]/g, "");
  const posledniCarka = cislo.lastIndexOf(",");
  const posledniTecka = cislo.lastIndexOf(".");
  if (posledniCarka > -1 && posledniTecka > -1) {
    // Oba oddělovače jsou v čísle přítomné — ten, co je víc vpravo, je desetinný
    // oddělovač, ten druhý je oddělovač tisíců a musí se úplně odstranit (aplikace
    // ho jinak omylem interpretovala jako desetinnou tečku, viz reálný export
    // z Flexi s anglickým formátem "4,700.00" — aplikace místo 4700 Kč četla 0).
    if (posledniCarka > posledniTecka) {
      // Evropský styl "1.234,50" — tečka je tisíce, čárka je desetinná
      cislo = cislo.replace(/\./g, "").replace(",", ".");
    } else {
      // Anglický styl "4,700.00" — čárka je tisíce, tečka je desetinná
      cislo = cislo.replace(/,/g, "");
    }
  } else if (posledniCarka > -1) {
    // Jen čárka v čísle — český zvyk, aplikace to bere jako desetinnou
    cislo = cislo.replace(",", ".");
  }
  // Jen tečka, nebo žádný oddělovač — aplikace to nechá beze změny, je to už platné číslo.
  return Number(cislo) || 0;
}

// Poskládá stejný popis a částku nákladu, jaké aplikace zapíše při importu — aplikace
...
// Aplikace to bere jako možnou duplicitu, když na zakázce už existuje náklad se
// STEJNÝM popisem a STEJNOU částkou — typicky proto, že aplikace stejnou fakturu
// (nebo stejný soubor) omylem naimportovala podruhé. Aplikace na to jen upozorní,
...
export function odhadnoutUdajeFaktury(radek) {
  ...
  // DŮLEŽITÉ: aplikace rozlišuje CENU ZA JEDNOTKU (tu je potřeba vynásobit množstvím,
  // ať aplikace nezapíše jen cenu za 1 kus místo celé položky) od CELKOVÉ částky za
  // řádek (tu aplikace použije přímo, beze změny). Sloupec "Cena za MJ" je přesně ten
  // první případ — aplikace to zjistila na reálném exportu z Flexi.
  ...
  if (castkaBezDph) {
    ...
    // aplikace cenu za jednotku vynásobí množstvím (výchozí množství 1, kdyby aplikace
...
// Tahle funkce to při čtení sjednotí, ať zbytek aplikace pracuje vždy jen s polem.
...
export function computeKalkulace(kalk, nastaveni, sazbaDph = DPH_SAZBA) {
  // Počet kusů násobí celou položku (materiál, práci, dopravu, kooperace...) — aplikace
...
export function rozpocitatNaklad(zakazky, castkaCelkem) {
  ...
  if (cenySoucet <= 0) {
    // Žádná ze zakázek nemá cenu — rozděl rovným dílem, ať to aplikace nespadne na dělení nulou.
...
// Seskupí odpracované hodiny podle DATA napříč VŠEMI zakázkami — aplikace to
// používá v kalendáři, ať u každého dne ukáže, kolik se ten den odvedlo na dílně
// a kolik na montáži. Dny bez jakékoliv práce se v mapě vůbec neobjeví (aplikace je
// tak nechá v kalendáři prázdné, ne s nulou).

// Plánované hodiny (z kalkulace — planCasDilna/planCasMontaz) přiřazené k týdnu
// PODLE TERMÍNU ZAHÁJENÍ VÝROBY dané zakázky — aplikace to bere jako "kolik práce
// má tenhle týden začít", ne kolik jí skutečně proběhne (aplikace nezná rozvržení
// práce uvnitř zakázky po dnech). Zakázky bez vyplněného termínu zahájení aplikace
// do přehledu nezahrne — nemá aplikace podle čeho poznat, do kterého týdne patří.
...
// stejný princip jako hodinyPodleDne, jen aplikace to bere z pole naklady místo
...
// "Z-2026-0008-2"... Počítá se z toho, co aplikace aktuálně vidí (podobně jako dřívější
...
// Spojí ručně zadané pracovníky (brigádníci) s e-maily VYBRANÝCH uživatelů aplikace
// (ne všech — aplikace nabídne jen ty, koho si admin v Nastavení zaškrtl) do jedné
// nabídky pro zápis práce — bez duplicit (i když se liší velikostí písmen).
// Když je nabízení v nastavení vypnuté, aplikace nevrátí nic.
