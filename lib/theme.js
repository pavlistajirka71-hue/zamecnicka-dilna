...
// Spojí ručně zadané pracovníky (brigádníci) s e-maily skutečných uživatelů appky
// do jedné nabídky pro zápis práce — bez duplicit (i když se liší velikostí písmen).
// Když je nabízení v nastavení vypnuté, appka nevrátí nic.
export function nabidkaPracovniku(nastaveni, uzivateleEmaily) {
  if (!nastaveni?.nabizetPracovniky) return [];
  const vse = [...(nastaveni.pracovnici || []), ...(uzivateleEmaily || [])];
