...
export function jeGoogleDriveNastaveny() {
  const maAutentizaci = !!process.env.GOOGLE_SERVICE_ACCOUNT_JSON || !!(process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL && process.env.GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY);
  return !!(maAutentizaci && process.env.GOOGLE_DRIVE_FOLDER_ID);
}
