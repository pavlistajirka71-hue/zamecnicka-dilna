...
U hotových nebo vyfakturovaných zakázek přibylo tlačítko **"Vygenerovat PDF a uložit na Drive"** v detailu zakázky. Appka sesbírá všechna podstatná data — zákazníka, popis, kalkulaci po položkách, zápisy práce, náklady/zisk/marži (u vyfakturovaných i s porovnáním s plánem), stav protokolu, materiál a vloží i fotky účtenek a fotodokumentace — a uloží to jako jeden PDF soubor do stejné sdílené složky na Google Drive jako ostatní fotky (nebo do Supabase Storage, pokud Drive nemáš nastavený — funguje to samo, bez dalšího nastavování). V appce zůstane jen odkaz, historie všech vygenerovaných archivů u zakázky se ukládá (jde vygenerovat víckrát, např. po každé aktualizaci).
