...
function PolozkaForm({ polozka, nastaveni, materialHistory, onChange }) {
  ...
  const addMaterial = () =>
    onChange({
      ...polozka,
      materialy: [...polozka.materialy, { id: uid(), nazev: "", dodavatel: "", cena: "", jednotka: "kg", mnozstvi: "", vaha: "", plocha: "" }],
    });
  const updateMaterial = (idx, item) => onChange({ ...polozka, materialy: polozka.materialy.map((m, i) => (i === idx ? item : m)) });
  const removeMaterial = (idx) => onChange({ ...polozka, materialy: polozka.materialy.filter((_, i) => i !== idx) });

  return (
    <div>
      <SectionLabel>Materiál</SectionLabel>
      {polozka.materialy.map((m, idx) => (
  ...
  return (
    <div>
      {polozky.map((p, idx) => {
        ...
        return (
          <div key={p.id} style={{ border: `1px solid ${C.line}`, borderRadius: 8, marginBottom: 10, overflow: "hidden" }}>
